...
- [x] **P0 — Cinematic Liquid v2 physical-scene v7 Vulkan 切片**：固定 128x64x96 MLS-MPM 网格、10 substeps 与 **320,920 粒子**（142x14x98 浅水底 + 48x37x71 溃坝体）；固定参数仍为 stiffness 45,000、viscosity 0.035、maxSpeed 8。身份为 `workloadVersion=cinematic_liquid_v2_physical_scene_v7`、`shaderVersion=9`、`sceneVersion=4`。用户调整后的母鸭、3 只小鸭以及 7 刚体 index ABI 均保留：船仍为 index 2、沉球仍为 index 3、小鸭仍追加在 4-6。
...
- [x] **P0 — 越沿流体与自适应表面**：池壁改为有限高度的内嵌碰撞壁（inset 0.22），外层 simulation catch band 允许粒子真实越沿后落到地面；catch band 带宽有限，**不是无限流体域**。5x5x5 binomial 重建改为自适应保留低支持喷滴。沉球入水 crown/whitewater 由真实 GPU 刚体状态与局部流体产生，不是 fragment 假水花；尚无独立 secondary-spray 粒子系统。
- [x] **P0 — 场景材质与环境**：前景侧壁新增独立透明软 PVC 薄膜，使用 IOR 1.50、Fresnel、弱吸收与 wrinkle；这只是专用薄膜近似，**不是完整 PVC 多介质 ray path**。render `pool.z`、PVC bottom、liner、草地与物理粒子底面已统一到 `y=0`，ring separation 由 `2*tube` 推导。场景加入无限程序化草地及大气天空/云，没有 cubemap 资产。v6 的 4 界面 Fresnel/Snell、分段 Beer–Lambert、opaque depth sorting、`extinction=(30,10,8)`、linear exposure 与 density 边界归零继续保留；未 vendor MIT [jeantimex/fluid](https://github.com/jeantimex/fluid) 的代码或资产。
- [x] **历史合同隔离**：`cinematic_liquid_v1`、正式 `cinematic_liquid_v2_surface_splat_optics_v4` / `shaderVersion=6`、`cinematic_liquid_v2_duck_family_v5` / `shaderVersion=7` 预览、`cinematic_liquid_v2_iterative_optics_v6` / `shaderVersion=8` 预览和当前 v7 必须各自分组，绝不混分。
...
- [x] **v2 下一重大画质路线 — SPH vertical slice 与视觉收口（2026-07-16）**：`--liquid-solver sph` → `cinematic_liquid_sph_slice_v1_preview`。当前深池版本为 **318,464 粒子**、计数排序邻域、per-particle SDF/浮力刚体耦合，复用 v7 池体、场景和用户鸭子家族。RTX 5090/Vulkan 已实际完整运行 15 秒，水池/水面/鸭子/球/船/草地稳定显示并正常自动结束，用户接受当前视觉并停止外观迭代。**这只完成视觉验收，不是正式成绩完成**；仍须修复 render-frame 驱动的 2×1/120 timestep、每 substep `bodyImpulses` 清零和 viscosity 原位 SSBO race，再做正式 15 秒 + 第 5 秒 RenderDoc、timestamp/确定性合同。secondary spray/foam、SPH 螺旋桨尾流与跨 API 是后续增强；细节见 HANDOFF 的 SPH vertical slice。
...
- [x] **CPU 补充测试 Windows vertical slice（2026-07-16）**：原生 `cpu_mixed_v1`、CLI `per-core|multi|all`、三轮中位数、独立 WinUI CPU 页、实时逐核/总进度、Run/Cancel、stdout 协议与 `results.json` summary 持久化已实现；不创建 3D 窗口、不调用 RenderDoc。9800X3D Release smoke 正确枚举 16 logical/8 physical/SMT2，逐核与 multi 全部 strict affinity、exit 0。正式合同为 15.0 秒总测量 + 0.2 秒预热 + r3；其他时长是 preview，版本同时隔离 affinity/time/warmup/sequence。详细逐核行只保留在当前页面/协议，JSON 保存逐核平均与 multi summary。
- [ ] **CPU 发布与平台合同收口**：重建新的 stage/ZIP/Inno Setup，并在干净 Windows 安装后验收 GUI 相邻 CLI 查找、Run/Cancel、History 写入；补 >64 logical/processor-group 与真实混合核实机。Linux allowed-cpuset + pthread affinity 当前只按 best-effort 记录，macOS 为 `scheduler_managed`/估计拓扑；Android/iOS/Web/WASM 未构建。P/E/Mid/LPE 只允许写 `Inferred*` 排名标签，不得宣称真实微架构识别，所有非 Windows strict 结果继续按 affinity capability 独立分组。
