...
// ---- Volumetric raymarch test (fullscreen triangle + raymarched fbm) ----
...
vertex VolVtxOut volumetricVertex(uint vid [[vertex_id]]) {
    VolVtxOut out;
    float2 uv = float2((vid << 1) & 2, vid & 2);
    out.uv = uv;
    // Metal NDC Y points up; flip to match Vulkan/GLSL convention.
...
float hash13_metal(float3 p) {
    p = fract(p * 0.1031);
    p += dot(p, p.zyx + 31.32);
    return fract((p.x + p.y) * p.z);
}

float noise3_metal(float3 x) {
    float3 i = floor(x);
    float3 f = fract(x);
    f = f * f * (3.0 - 2.0 * f);

    float n000 = hash13_metal(i + float3(0.0, 0.0, 0.0));
    float n100 = hash13_metal(i + float3(1.0, 0.0, 0.0));
    float n010 = hash13_metal(i + float3(0.0, 1.0, 0.0));
    float n110 = hash13_metal(i + float3(1.0, 1.0, 0.0));
    float n001 = hash13_metal(i + float3(0.0, 0.0, 1.0));
    float n101 = hash13_metal(i + float3(1.0, 0.0, 1.0));
    float n011 = hash13_metal(i + float3(0.0, 1.0, 1.0));
    float n111 = hash13_metal(i + float3(1.0, 1.0, 1.0));
...
fragment float4 volumetricFragment(VolVtxOut in [[stage_in]],
    ...
    for (uint i = 0u; i < params.steps; ++i) {
        float3 p = ro + rd * t;
        float w = fbm_metal(p * 0.5 + float3(0.0, params.time * 0.05, 0.0));
        float d = fbm_metal(p * 0.5 + w * 0.6);
    ...
    float3 col = mix(cold, hot, v) * (1.0 - trans) + float3(0.05, 0.08, 0.13) * trans;
    return float4(col, 1.0);
}
